...
\textit{Catatan:} Evaluasi HMM dilakukan pada \textbf{simulasi time-lapse terkontrol}, bukan pada data time-lapse nyata, karena dataset BBBC048 merupakan snapshot cross-sectional (bukan sekuens temporal). Simulasi dilakukan dengan mensintesis sekuens fase dari ODE ground-truth dan menambahkan noise klasifikasi 18\% (menggantikan prediksi dengan fase acak) untuk merepresentasikan error CNN yang realistis. Ini merupakan \textit{proof-of-concept} yang mendemonstrasikan prinsip koreksi HMM sebelum validasi pada data time-lapse nyata.
...
\begin{enumerate}[noitemsep,topsep=3pt]
    ...
    \item \textbf{Peningkatan konsistensi sekuens +$\sim$15\% pada proof-of-concept simulasi:} Koreksi HMM dengan matriks transisi dari ODE meningkatkan akurasi sekuensial dari $\sim$82\% (CNN-only) menjadi $\sim$97\%, dan mengurangi transisi tidak valid sebesar $\sim$90\%, pada simulasi terkontrol dengan noise rate 18\%. Validasi pada data time-lapse nyata merupakan langkah penelitian berikutnya.
